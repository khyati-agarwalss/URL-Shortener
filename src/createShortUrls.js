...
const createErrorDescription = ((code, err) => {
    switch(code) {
        ...
        case 401: return 'Unauthorized: Be sure you configured the integration to use a valid API key';
        case 403: return `Invalid request: ${err.source} ${err.message}`;
        case 404: return `Not found: ${err.source} ${err.message}`;
        case 503: return 'Short URL service currently under maintenance. Try again after some time';
        default: return 'Unexpected error occurred while connecting to Rebrandly APIs';
    }
});
...
const createShortUrlsFactory = ((apikey) => (urls, domain, slashtags) => {
    ...
    if(Array.isArray(slashtags)) {
        slashtags.forEach((slashtags, i) => (structuredUrls.slashtag = slashtag));
    }
